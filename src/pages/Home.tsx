...
const CyberBackground = () => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // First frame: solid fill so canvas is opaque immediately
      if (frame === 1) {
      ...
      // draw node dots
      for (const nd of nodes) {
        const grd = ctx.createRadialGradient(nd.x, nd.y, 0, nd.x, nd.y, nd.r * 3);
        grd.addColorStop(0, nd.color + "0.85)");
        grd.addColorStop(1, nd.color + "0)");
        ctx.fillStyle = grd;
        ctx.beginPath();
...
const Home = () => {
  ...
  return (
    <div className="min-h-screen dark flex relative overflow-hidden" style={{ background: "rgb(8,10,20)" }}>
      <CyberBackground />
